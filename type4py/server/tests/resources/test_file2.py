# THIS IS A TEST FILE FOR TESTING THE SERVER's PREDICT API
...
LOCAL_CONFIGURATION_FILE = ".pyre_configuration.local"
...
def get_relative_local_root(global_root: pathlib.Path,
                            local_root:Dict[pathlib.Path, PackageSummary]) -> Optional[str]:
...
def find_parent_directory_containing_file(base: str, target:str) -> Optional[Path]:
    """
...
def find_typeshed_search_paths(typeshed_root:pathlib.Path) -> List[Path]:
    """
