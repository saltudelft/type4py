...
def load_combined_train_data(output_path: str):
    return torch.cat(
        (load_data_tensors_TW(join(output_path, 'vectors', 'train', 'identifiers_param_train_datapoints_x.npy')),
...
def load_combined_train_data_split(output_path: str, type: str):
    if type == "var":
        ...
        logger.info("Type set not defined or not found...")


def load_combined_valid_data(output_path: str):
    return torch.cat(
        (load_data_tensors_TW(join(output_path, 'vectors', 'valid', 'identifiers_param_valid_datapoints_x.npy')),
...
def load_combined_labels_split(output_path: str, type: str):
    if type == "var":
        return load_data_tensors_TW(join(output_path, 'vectors', 'train', 'var_train_dps_y_all.npy')), \
               load_data_tensors_TW(join(output_path, 'vectors', 'valid', 'var_valid_dps_y_all.npy')), \
               load_data_tensors_TW(join(output_path, 'vectors', 'test', 'var_test_dps_y_all.npy'))
    elif type == "ret":
        return load_data_tensors_TW(join(output_path, 'vectors', 'train', 'ret_train_dps_y_all.npy')), \
               load_data_tensors_TW(join(output_path, 'vectors', 'valid', 'ret_valid_dps_y_all.npy')), \
               load_data_tensors_TW(join(output_path, 'vectors', 'test', 'ret_test_dps_y_all.npy'))
    elif type == "param":
        return load_data_tensors_TW(join(output_path, 'vectors', 'train', 'params_train_dps_y_all.npy')), \
               load_data_tensors_TW(join(output_path, 'vectors', 'valid', 'params_valid_dps_y_all.npy')), \
               load_data_tensors_TW(join(output_path, 'vectors', 'test', 'params_test_dps_y_all.npy'))
...
def load_training_data_per_model(data_loading_funcs: dict, output_path: str,
    ...
    if data_loading_funcs['name'] == 'woi':
        ...
        logger.info(
            f"Loaded train set of the {data_loading_funcs['name']} dataset in {(time() - load_data_t) / 60:.2f} min")
...
def load_test_data_per_model(data_loading_funcs: dict, output_path: str,
    ...
    logger.info(
        f"Loaded the test set of the {data_loading_funcs['name']} dataset in {(time() - load_data_t) / 60:.2f} min")
